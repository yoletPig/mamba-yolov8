...
from ultralytics.nn.modules.utils import channel_shuffle

from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad
from .transformer import TransformerBlock
from .mamba.vmamba import VSSBlock
...
class C2DMSC(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""

    def __init__(self, c1, c2, n=1, shortcut=False, g=0, e=0.5):
        """Initialize CSP bottleneck layer with two convolutions with arguments ch_in, ch_out, number, shortcut, groups,
        expansion.
        """
        super().__init__()
        ...
        self.m = nn.ModuleList(
            DynamicMultiScaleConvolution(
                self.c, 
                stride=1, 
                num_branch=g, 
                num_groups=[2, 2, 2, 4],
                num_kernels=[4, 4, 2, 1]) for _ in range(n))
        self.vssblock = VSSBlock(hidden_dim=self.c)

    def forward(self, x):
        """Forward pass through C2f layer."""
        x_main = self.cv1(x)
        x_main = list(x_main.split((self.c, self.c), 1))
        x_vss = x_main[0].permute(0,2,3,1).contiguous()
        x_vss = self.vssblock(x_vss)
        x_main[0] =  x_vss.permute(0,3,1,2).contiguous()
        x_main.extend(blocks(x_main[-1]) for blocks in self.m)
        return self.cv2(torch.cat(x_main, 1))
...
class DynamicMultiScaleConvolution(nn.Module):
    def __init__(self, channels, stride, num_branch, num_groups, num_kernels, with_cp=False):
        super().__init__()

        self.with_cp = with_cp

        self.num_groups = num_groups[num_branch] # 2
        self.num_kernels = num_kernels[num_branch] # 4
